...
const emit = defineEmits<{
  (e: "confirm", parameters: DocumentParameters): void;
}>();
...
const parameters = ref<DocumentParameters>({});
...
<template>
  <v-dialog v-model="dialog" @keydown.esc="close" width="600">
    <template v-slot:activator="{ props }">
      <v-btn v-bind="props" color="primary">Generate document</v-btn>
    </template>

    <v-form validate-on="submit" @submit.prevent="submit">
      <v-card>
        <v-card-title>Generate method list document</v-card-title>
